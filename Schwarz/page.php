<?php
    ...
    $page = min($page,$max_p);
    $start = ($page - 1) * 10;
    $s2 = mysql_query('SELECT * FROM member_status WHERE member_id="'.mysql_real_escape_string($ss1["id"]).'" ORDER BY status_id DESC LIMIT '.$start.',10;');
?>
<!DOCTYPE html>
    <head>
        <meta charset="utf-8">
    ...
    <body>
        ...
        <div id="header">
            ...
            <input type="button" id="nttnp" class="search_button" value="検索">
        ...
        <div id="content">
                <p class="blank"></p>
                    <div style="float:right;">
                     ...
                     　  <button class="btn" style="width:190px;" onClick="location.href='tag.php?id=<?php print(htmlspecialchars($_GET['id'])); ?>';">まとめタグ</button><br>
                     　  <button class="btn" style="width:190px;" onClick="location.href='video.php?id=<?php print(htmlspecialchars($_GET['id'])); ?>';">動画サイト</button><br>
                     　  <button class="btn" style="width:190px" onClick="location.href='image.php?id=<?php print(htmlspecialchars($_GET['id'])); ?>';">画像</button>
                     　  <br>
                     　 <br>
                        ...
                        if($ctt["tn"] > 0){
                            while($data = mysql_fetch_assoc($s2)){
                                ...
                                if(isset($_SESSION["user_id"]) && $query == $_SESSION["user_id"]) {
                                    $output .= ' / <a style="color:#615555;" href="edit.php?id='.htmlspecialchars($data["status_id"]).'">編集</a>';
                                }
                                $output .= '</span></span></p>';
                                $output .= '<br>';
                            }
                        ...
                        <div id="paging">
                            <?php if($page > 1) { ?>
                                <p style="margin:0;float:left;">
                                    <a href="page.php?id=<?php print(htmlspecialchars($_GET['id'])) ?>&page=<?php print($page-1); ?>">前のページへ</a>
                                </p>
                            <?php } else { ?>
                                <p style="margin:0;float:left;">
                                    前のページへ
                            ...
                            <?php if($page < $max_p) { ?>
                                 <p style="margin:0;float:right;">
                                    <a href="page.php?id=<?php print(htmlspecialchars($_GET['id'])) ?>&page=<?php print($page+1); ?>">次のページへ</a>
                                </p>
                            <?php } else { ?>
                                 <p style="margin:0;float:left;">
                                    次のページ
